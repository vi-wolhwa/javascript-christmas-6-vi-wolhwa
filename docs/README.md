...
### 3-1. Deep Freezing 모듈

- ✅ 객체/배열에 대하여 Deep Freezing 수행하는 모듈

### 3-2. Deep Copy 모듈

- ✅ 객체/배열에 대하여 Deep Copy를 수행하는 모듈
...
- ✅ 메시지 (outputView)
  ...
  - result
    - title
      ...
      - `<12월 이벤트 배지>`
    - content
      (amountFormat : 천 단위 구분자)
      - nothing : `없음`
      - order_menu : `${name} ${count}개`,
      - total_price : `${amountFormat(amount)}원`,
      - giveaway : `${name} ${count}개`,
      - benefit_details : `${name}: -${amountFormat(amount)}원`,
      - total_benefits : `-${amountFormat(amount)}원`,
      - discounted_price : `${amountFormat(amount)}원`,
      - event_badge : `${name}`
